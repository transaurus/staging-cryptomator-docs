...
If the network connection is present, it is used for optional features, i.e. update checks and searching the error database for solutions. The only exception is when unlocking [Cryptomator Hub](/zh-Hans/hub/introduction/.md) vaults, then a network connection to the hub server is required. All network connections to the internet are using HTTPS with at least TLS 1.2.

## Trust Certificate Management[​](#trust-certificate-management "Direct link to Trust Certificate Management")
...
To change the proxy server, you need to edit `Cryptomator.cfg` located in the installation/app directory. Open the file in a text editor, search for the line:
