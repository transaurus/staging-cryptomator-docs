...
Vaults mounted to a drive letter are only accessible to the current user. If you want to access the vault as a different/elevated user, you have to use WinFsp (Local Drive) and [mount to a directory](/desktop/vault-management/.md#vault-options).
...
`allow_root` and `allow_other` cannot be used as [custom mount flags](/desktop/vault-management/.md#vault-options) without enabling (uncommenting) `user_allow_other` option in **/etc/fuse.conf** configuration file.
